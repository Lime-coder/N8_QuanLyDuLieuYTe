...
-- M?c dích:
-- 1. Gi? TC#5: nhân viên query tr?c ti?p STAFF ch? th?y chính mình.
-- 2. T?o b?ng ph? t?i thi?u cho Ði?u ph?i viên ch?n Bác si/Y si và K? thu?t viên.
-- Run as: HOSPITAL_DBA có quy?n DBMS_RLS
-- ==============================================================================
...
BEGIN
    ...
    -- Bypass cho schema owner, DBA app, và khi trigger ch?y (CURRENT_USER = 'HOSPITAL')
    IF SYS_CONTEXT('USERENV', 'CURRENT_USER') IN ('HOSPITAL', 'HOSPITAL_DBA') THEN
        RETURN '1=1';
    END IF;

    -- Nhân viên ch? th?y / s?a dòng c?a chính mình
...
BEGIN
    -- Policy 1: SELECT tr?c ti?p STAFF ch? th?y chính mình
    DBMS_RLS.ADD_POLICY(
    ...
    -- Policy 2: UPDATE phone, hometown ch? trên dòng chính mình
    DBMS_RLS.ADD_POLICY(
        object_schema     => 'HOSPITAL',
        object_name       => 'STAFF',
...
-- ==============================================================================
-- PH?N 5: T?O B?NG PH? T?I THI?U CHO ÐI?U PH?I VIÊN PHÂN CÔNG
-- ==============================================================================
...
-- ==============================================================================
-- PH?N 6: Ð? D? LI?U T?I THI?U T? STAFF SANG B?NG PH?
-- ==============================================================================
...
-- ==============================================================================
-- PH?N 7: T?O VIEW CHO ÐI?U PH?I VIÊN
-- ==============================================================================
...
WHERE staff_role IN (N'Bác si', N'Bác si/Y si');
...
SELECT
    ...
    full_name
FROM hospital.COORD_ASSIGNMENT_STAFF
WHERE staff_role = N'K? thu?t viên';


-- ==============================================================================
-- PH?N 8: CH?NH S?A SCHEMA CHO PHÉP ÐI?U PH?I VIÊN PHÂN CÔNG (ALLOW NULL TECHNICIAN)
-- ==============================================================================

-- 8.1 Cho phép c?t technician_id nh?n giá tr? NULL d? luu các d?ch v? chua phân công
BEGIN
    EXECUTE IMMEDIATE 'ALTER TABLE hospital.service_record MODIFY technician_id NULL';
EXCEPTION
    WHEN OTHERS THEN
        IF SQLCODE = -1451 THEN
            NULL; -- B? qua n?u c?t dã cho phép NULL
        ELSE
            RAISE;
        END IF;
END;
...
-- 8.2 C?p nh?t trigger d? b? qua ki?m tra KTV n?u technician_id là NULL
...
BEGIN
    IF :NEW.technician_id IS NOT NULL THEN
        ...
        IF v_tech_active = 0 THEN
            RAISE_APPLICATION_ERROR(-20012, 'Không th? t?o/c?p nh?t d?ch v?: K? thu?t viên này dã b? khóa (Không ho?t d?ng).');
        END IF;
    END IF;
END;
/

-- 8.3 Thêm m?t record m?u chua phân công l?y t? HSBA có s?n d? tránh l?i Khóa ngo?i
BEGIN
    FOR r IN (SELECT record_id FROM hospital.medical_record WHERE ROWNUM = 1) LOOP
        ...
        SELECT r.record_id, N'Xét nghi?m máu', SYSDATE - 1, NULL, NULL
        FROM DUAL
        WHERE NOT EXISTS (
            SELECT 1 FROM hospital.service_record WHERE record_id = r.record_id AND service_type = N'Xét nghi?m máu'
        );
    END LOOP;
    COMMIT;
END;
...
-- STAFF:
-- Query tr?c ti?p STAFF s? b? VPD l?c ch? th?y chính mình
GRANT SELECT ON hospital.staff TO rl_coordinator;

-- Ch? du?c c?p nh?t thông tin cá nhân h?p l?
GRANT UPDATE (phone, hometown) ON hospital.staff TO rl_coordinator;

-- View ph?c v? di?u ph?i bác si/k? thu?t viên
...
-- M?c dích: Stored Procedures thay th? cho raw SQL trong CoordinatorRepository
-- Run as: HOSPITAL_DBA
-- ==============================================================================
...
-- 4. Thông tin cá nhân (Ði?u ph?i viên)
...
BEGIN
    ...
    VALUES (p_record_id, p_patient_id, p_record_date, p_doctor_id, p_dept_id, N'Chua ch?n doán', N'Chua di?u tr?', N'Chua k?t lu?n');
    COMMIT;
END;
/
...
-- B?nh nhân ch? th?y d? li?u c?a chính mình
-- ------------------------------------------------------------
...
-- USP_GET_PATIENT_PROFILE: L?y thông tin cá nhân b?nh nhân dang dang nh?p
...
-- USP_GET_PATIENT_RECORDS: L?y danh sách h? so b?nh án c?a b?nh nhân
...
-- USP_UPDATE_PATIENT_CONTACT: B?nh nhân t? c?p nh?t thông tin liên l?c và ti?n s? b?nh lý (TC#5)
...
WHERE UPPER(ST.USERNAME_DB) = SYS_CONTEXT('USERENV', 'SESSION_USER')
  AND ST.STAFF_ROLE = N'K? thu?t viên'
  AND ST.IS_ACTIVE = 1;
...
BEGIN
    ...
    IF V_ROWS_UPDATED = 0 THEN
        RAISE_APPLICATION_ERROR(
            -20031,
            N'Không du?c phép c?p nh?t: d?ch v? không thu?c k? thu?t viên hi?n t?i.'
        );
    END IF;
END;
/ 
...
BEGIN
    ...
    IF V_ROWS_UPDATED = 0 THEN
        RAISE_APPLICATION_ERROR(-20032, N'Không tìm th?y k? thu?t viên hi?n t?i ho?c không có quy?n c?p nh?t.');
    END IF;
END;
/
...
BEGIN
    ...
    IF NOT (
        v_staff_role IN (N'Bác si', N'Bác si/Y si')
...
BEGIN
    ...
    IF SQL%ROWCOUNT = 0 THEN
        RAISE_APPLICATION_ERROR(-20002, N'L?i: Không tìm th?y h? so b?nh án ho?c bác si không có quy?n c?p nh?t h? so này.');
    END IF;
END;
/
...
BEGIN
    ...
    IF v_count = 0 THEN
        RAISE_APPLICATION_ERROR(-20001, N'Mã HSBA không t?n t?i ho?c bác si không có quy?n t?o d?ch v? cho HSBA này.');
    END IF;
...
EXCEPTION
    WHEN DUP_VAL_ON_INDEX THEN
        RAISE_APPLICATION_ERROR(-20006, N'L?i: D?ch v? này dã t?n t?i cho HSBA/ngày hi?n t?i.');
END;
/
...
BEGIN
    ...
    IF SQL%ROWCOUNT = 0 THEN
        RAISE_APPLICATION_ERROR(-20007, N'L?i: Không tìm th?y d?ch v? ho?c bác si không có quy?n xóa d?ch v? này.');
    END IF;
END;
/
...
BEGIN
    IF v_action NOT IN ('INSERT', 'UPDATE', 'DELETE') THEN
        RAISE_APPLICATION_ERROR(-20008, N'L?i: Hành d?ng don thu?c không h?p l?.');
    END IF;
    ...
    IF v_action IN ('INSERT', 'UPDATE') THEN
        ...
        IF v_count = 0 THEN
            RAISE_APPLICATION_ERROR(-20003, N'L?i: Mã HSBA không t?n t?i ho?c bác si không có quy?n kê don cho HSBA này.');
        END IF;
    END IF;
    ...
    IF v_action = 'INSERT' THEN
        ...
        IF v_count > 0 THEN
            RAISE_APPLICATION_ERROR(-20005, N'L?i: Thu?c này dã du?c kê trong don c?a ngày hôm nay.');
        END IF;
    ...
    ELSIF v_action = 'UPDATE' THEN
        IF p_date IS NULL OR p_old_med_name IS NULL THEN
            RAISE_APPLICATION_ERROR(-20009, N'L?i: C?p nh?t don thu?c c?n ngày kê don và tên thu?c cu.');
        END IF;
        ...
        IF SQL%ROWCOUNT = 0 THEN
            RAISE_APPLICATION_ERROR(-20010, N'L?i: Không tìm th?y don thu?c ho?c bác si không có quy?n c?p nh?t.');
        END IF;

    ELSIF v_action = 'DELETE' THEN
        IF p_date IS NULL THEN
            RAISE_APPLICATION_ERROR(-20011, N'L?i: Xóa don thu?c c?n ngày kê don.');
        END IF;
        ...
        IF SQL%ROWCOUNT = 0 THEN
            RAISE_APPLICATION_ERROR(-20012, N'L?i: Không tìm th?y don thu?c ho?c bác si không có quy?n xóa.');
        END IF;
    END IF;
END;
/
...
BEGIN
    ...
    IF SQL%ROWCOUNT = 0 THEN
        RAISE_APPLICATION_ERROR(-20004, N'L?i: Không tìm th?y b?nh nhân ho?c bác si không có quy?n c?p nh?t b?nh nhân này.');
    END IF;
END;
/
...
BEGIN
    ...
    IF SQL%ROWCOUNT = 0 THEN
        RAISE_APPLICATION_ERROR(-20013, N'L?i: Không tìm th?y thông tin bác si hi?n t?i ho?c tài kho?n dã b? khóa.');
    END IF;
END;
/
...
-- Ð?t sau PACKAGE BODY
-- G?i d?ng d? tránh l?i PLS-00905 khi package t?m INVALID lúc compile
-- ==============================================================================
