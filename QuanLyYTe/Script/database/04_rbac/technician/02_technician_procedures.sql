...
AS
BEGIN
    UPDATE hospital_dba.SERVICE_RECORD SR
    ...
    WHERE SR.RECORD_ID = P_RECORD_ID
      ...
      -- BẢO MẬT: Phải tự nối bảng STAFF để chặn không cho KTV update hồ sơ của người khác!
      AND SR.TECHNICIAN_ID = (
          SELECT STAFF_ID FROM hospital.STAFF 
