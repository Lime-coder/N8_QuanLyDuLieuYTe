...
BEGIN
    OPEN p_cursor FOR
        SELECT 'HOSPITAL_DBA' AS USERN_NAME FROM DUAL
        UNION
        SELECT UPPER(username_db) AS USERN_NAME FROM hospital.staff   WHERE username_db IS NOT NULL
        UNION
        SELECT UPPER(username_db) AS USERN_NAME FROM hospital.patient WHERE username_db IS NOT NULL
        ORDER BY USERN_NAME;
END USP_GET_USERS;
/
...
BEGIN
    OPEN p_cursor FOR
        -- System privileges
        SELECT 'SYSTEM' AS privilege_type, sp.PRIVILEGE AS privilege,
               NULL AS owner, NULL AS object_name, NULL AS column_name,
        ...
        -- Table/view/procedure/function-level object privileges
        SELECT
            CASE ao.OBJECT_TYPE
                ...
                ELSE ao.OBJECT_TYPE
            END,
            tp.PRIVILEGE, tp.TABLE_SCHEMA, tp.TABLE_NAME, NULL, tp.GRANTABLE
        FROM ALL_TAB_PRIVS tp
        JOIN ALL_OBJECTS ao ON ao.OWNER = tp.TABLE_SCHEMA AND ao.OBJECT_NAME = tp.TABLE_NAME
        ...
        SELECT 'COLUMN', cp.PRIVILEGE, cp.TABLE_SCHEMA, cp.TABLE_NAME, cp.COLUMN_NAME, cp.GRANTABLE
